...
class Collector(private val redis: Jedis) {
    ...
    // Fetch top news from NewsAPI (with Redis caching)
    ...
    // Fetch news by topic (with Redis caching)
    ...
    // Fetch news by query (with Redis caching)
...
fun main() {
    ...
    // Direct Jedis connection using environment variables
    val redis = Jedis(redisHost, redisPort)

    // Create Collector with the Jedis instance
